...
fn path_name(path: &Path) -> &'static str {
    match path {
		...
		Path::ChannelsIdThreadMembersId(_) => "Thread member",
		...
		Path::GuildsIdAutoModerationRules(_) => "Guild automoderation rules",
		Path::GuildsIdAutoModerationRulesId(_) => "Guild automoderation rule",
		...
		Path::GuildsIdMfa(_) => "Guild MFA setting",
